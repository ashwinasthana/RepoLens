...
const TABS = [
  ...
  { id:'roast',       icon:'ti-flame',            label:'Roast'       },
...
function generateReport(file, content, cache) {
  ...
  const roastValue = typeof cache?.roast === 'string' ? cache.roast : cache?.roast?.roast
  if (roastValue) md += `## Roast\n${roastValue}\n\n`
...
const AI_TAB_TASKS = {
  ...
  roast: { label: 'Roast', type: 'ROAST_FILE' },
  onboarding: { label: 'Onboarding', type: 'ONBOARD_FILE' },
}
const AI_TABS = new Set(Object.keys(AI_TAB_TASKS))
const AI_MESSAGE_TYPES = new Set(['ANALYZE_FILE', 'EXPLAIN_FILE', 'GRAPH_FILE', 'DEFS_FILE', 'ROAST_FILE', 'ONBOARD_FILE', 'CHAT_REPO', 'CHAT'])
...
function renderTabBody(tab, content, cache) {
  ...
  switch (tab) {
    ...
    case 'roast': {
      const r = cache?.roast
      if (!r) return skeletonHTML(5)
      if (typeof r === 'object' && r.loading) return skeletonHTML(5)
      if (typeof r === 'object' && r.error) return renderTabError(r.error)

      const roastText = typeof r === 'string' ? r : (r.roast || '')
      ...
      return `
        ${rating ? `<div class="purpose-line"><i class="ti ti-temperature"></i> Heat level: ${escapeHtml(rating)}</div>` : ''}
        <div class="section">
          <div class="section-label"><i class="ti ti-flame"></i> Repo roast</div>
          <div class="prose-card markdown-panel">${renderMarkdown(roastText || 'No roast available yet.')}</div>
        </div>
        ${highlights.length ? `
        <div class="section">
          <div class="section-label"><i class="ti ti-alert-triangle"></i> Savage callouts</div>
          <ul style="padding-left:18px;color:var(--muted);font-size:12px;display:flex;flex-direction:column;gap:4px;">
            ${highlights.map(item => `<li>${escapeHtml(item)}</li>`).join('')}
          </ul>
        </div>` : ''}
        ${fixes.length ? `
        <div class="section">
          <div class="section-label"><i class="ti ti-tool"></i> How to clap back</div>
          <ul style="padding-left:18px;color:var(--muted);font-size:12px;display:flex;flex-direction:column;gap:4px;">
